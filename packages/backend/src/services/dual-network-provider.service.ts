...
export class DualNetworkProvider {
  ...
  > {
    try {
      ...
      console.log(`✓ Storage upload complete - Root: ${merkleRoot.substring(0, 16)}...`);
      ...
      console.log(`✓ AI inference complete - TEE verified: ${aiResult.ok.verificationComponents.zgTeeVerified}`);
      ...
      const diagnosisTx = await this.sepoliaMedicalRegistry.storeDiagnosis(
        ...
        encryptedDiagnosis,
        diagnosisProof,
        aiResult.ok.verificationComponents.zgTeeSignature,
        { gasLimit: 3_000_000 }
      );
      ...
      return {
        ok: {
          diagnosisId: aiResult.ok.verificationComponents.zgRequestId,
          diagnosis: aiResult.ok.diagnosis,
          confidence: aiResult.ok.confidence,
          verificationComponents: aiResult.ok.verificationComponents,
          networks: {
            sepolia: {
            ...
            og: {
              ...
              computeRequestId: aiResult.ok.verificationComponents.zgRequestId,
              storageTx: txHash
            }
          }
        }
